...
// Téléportation de case 7 vers 13 avec disparition temporaire du pion
                     ...
                     bool *tpActive, float *tpTimer, bool *tpInvisible)
{
    ...
    // On lance le timer et on rend le pion invisible
    if (!(*tpActive)) {
        *tpActive = true;
        *tpTimer = pauseTime;
        *tpInvisible = true;
    ...
    // Tant que le timer tourne, on affiche le message (pion invisible)
    if (*tpTimer > 0.0f) {
        *tpTimer -= GetFrameTime();
        ...
        DrawTextEx(font, txt, (Vector2){ x, y }, size, 0,
                   (Color){ 180, 40, 200, 255 });
        return;
    }

    // Fin du timer donc on réapparaît à la case 13
    if (!player->isMoving) {
        player->pos = 13;     // téléportation immédiate
        *tpInvisible = false; // on réaffiche le pion
        *tpActive = false;    // reset du piège
    }

    // Message pendant la frame de réapparition
    const char *txt = "TELEPORTATION MAGIQUE !";
    int size = 55;
