...
int main(void) {
    ...
    // Ajout de la texture pour le cadeau de début
    Texture2D cadeau = LoadTexture("cadeaux.png");
...
//--------------------------------------------------------------
{
    ...
    for (int i = 0; i < lightsCount; i++) {
        ...
        case STATE_BOARD: {
            ...
            // Cadeau au début du plateau
            Vector2 posCadeau = {35, 170}; // position x,y sur l’écran
            float scaleCadeau = 0.4f;               // ajuster la taille
            DrawTextureEx(cadeau, posCadeau, 0.0f, scaleCadeau, WHITE);

            // Traineau de Noël en bas à gauche
            Vector2 posTraineau = {1100, 790}; // position x,y sur l’écran
