...
int main(void) {
    ...
    float arriveDelay = 0.0f;
    int pendingMiniGame = 0;   // 0 = rien, 1 = sudoku, 2 = morpion, 3 = pendu, 4 = demineur
    ...
    while (!WindowShouldClose()) {
        ...
        case STATE_BOARD: {
            ...
            if (player.isMoving) {
                ...
                // Si on vient d'arriver sur une case ce frame :
                if (arrived) {
                    // On regarde sur quelle case on est arrivé et on prépare le mini-jeu
                    if (player.pos == 1 || player.pos == 10 || player.pos == 17) {
                        pendingMiniGame = 1;   // Sudoku
                        arriveDelay = 0.15f;   // 150 ms avant de lancer
                    } else if (player.pos == 8 || player.pos == 14) {
                        pendingMiniGame = 2;   // Morpion
                        arriveDelay = 0.15f;
                    } else if (player.pos == 3 || player.pos == 11 || player.pos == 16) {
                        pendingMiniGame = 3;   // Pendu
                        arriveDelay = 0.15f;
                    } else if (player.pos == 2 || player.pos == 6 || player.pos == 13) {
                        pendingMiniGame = 4;   // Démineur
                        arriveDelay = 0.15f;
                    }
                }
            }
            ...
            } // Gestion du délai d'arrivée pour lancer les mini-jeux
            if (pendingMiniGame != 0) {

                arriveDelay -= GetFrameTime();  // on descend le timer

                if (arriveDelay <= 0.0f) {

                    int mg = pendingMiniGame;
                    pendingMiniGame = 0; // reset

                    EndDrawing(); // IMPORTANT : il faut terminer le dessin avant d’ouvrir un autre écran

                    if (mg == 1) {
                        // SUDOKU
                        if (player.pos == 1)  SudokuSetLevel(1);
                        if (player.pos == 10) SudokuSetLevel(2);
                        if (player.pos == 17) SudokuSetLevel(3);
                        StartSudoku(font);
                    } else if (mg == 2) {
                        // MORPION
                        system("morpion.exe");
                    ...
                    continue; // on retourne dans la boucle principale
                }
            }
